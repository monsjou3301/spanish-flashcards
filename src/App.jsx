...
import { Plus, RotateCw, Flame, X, ChevronRight, BookOpen, Sparkles, Upload, Trash2, ArrowRight, Settings, Languages, AlertCircle, Pencil } from 'lucide-react';
...
export default function App() {
  ...
  // ==============================
  // Edit current card (updates both directions in this pair)
  // ==============================
  const handleEdit = (newFront, newBack) => {
    const card = queue[currentIdx];
    if (!card || !newFront.trim() || !newBack.trim()) return;
    const pk = activePairKey;
    const pair = pairs[pk];
    const front = newFront.trim();
    const back = newBack.trim();

    // Update both direction cards for this vocab item in the deck
    const newDeck = decks[pk].map(c => {
      if (c.vocabId !== card.vocabId) return c;
      if (c.dir === card.dir) return { ...c, front, back };
      // mirror card: front/back are swapped
      return { ...c, front: back, back: front };
    });
    const newDecks = { ...decks, [pk]: newDeck };
    setDecks(newDecks);
    storage.set('decks', newDecks);

    // Also update the underlying pair.vocab so future top-ups stay consistent.
    // vocabId format is "v<index>"; pair.vocab[index] has {a, b} where a is from-lang.
    const idx = parseInt(card.vocabId.slice(1), 10);
    if (!isNaN(idx) && pair.vocab[idx]) {
      const newVocab = [...pair.vocab];
      // For the forward direction card, front=a and back=b. For backward, front=b and back=a.
      newVocab[idx] = card.dir === 'fwd' ? { a: front, b: back } : { a: back, b: front };
    ...
    // Refresh the queue with updated card
...
// CARD VIEW
// ===================================================================
function CardView({ card, currentIdx, queueLength, flipped, onFlip, onRate, onTrash, onEdit }) {
  ...
  return (
    <>
      ...
      {/* Direction indicator + edit + trash */}
      <div className="flex items-center justify-between mb-3 px-1">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 rounded-full text-stone-600 hover:text-amber-300 hover:bg-amber-950/30 transition-colors"
...
function EditCardModal({ card, onClose, onSave }) {
  ...
  const fromMeta = getLangMeta(card.fromLang);
  const toMeta = getLangMeta(card.toLang);
  const unchanged = front.trim() === card.front && back.trim() === card.back;

  return (
    <ModalShell onClose={onClose} title="Edit card">
      <div className="space-y-3">
        <div>
          <label className="text-[10px] tracking-[0.25em] uppercase text-stone-500">
            {fromMeta.flag} {fromMeta.name} (front)
          </label>
          <input
            autoFocus
            value={front}
      ...
      <button
        ...
        disabled={!front.trim() || !back.trim() || unchanged}
        className="w-full mt-5 py-3 bg-amber-100 text-stone-950 rounded-xl font-medium hover:bg-amber-200 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        Save <ChevronRight size={16} />
      </button>
      <p className="text-xs text-stone-600 mt-3 text-center">
        Updates this card and its reverse direction
      </p>
    </ModalShell>
  );
}
